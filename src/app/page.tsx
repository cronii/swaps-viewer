      ...
      <table>
        ...
        <tbody>
          {swapsData.map((swap: Swap) => {
            ...
            return (
              <tr key={tx_id} className='text-right'>
                <td>{block}</td>
                ...
                {isBuy && (<td className='text-green-500'>{displayToken(token0_out)}</td>)}
                {isBuy && (<td className='text-green-500'>{displayToken(token1_in)}</td>)}
                {!isBuy && (<td className='text-red-500'>{displayToken(token0_in)}</td>)}
                {!isBuy && (<td className='text-red-500'>{displayToken(token1_out)}</td>)}
              </tr>
            )
          })}
        </tbody>
